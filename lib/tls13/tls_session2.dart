...
class TLS13Session {
  ...
  Future<void> connect() async {
    ...
    // ✅ Transcript already updated by _recvEncryptedHandshake()
    // Do NOT hash plaintext again.
  ...
  //   while (true) {
  //     // Receive an encrypted handshake record
  ...
  //     // Strip the final content‑type byte (0x16)
  //     final fragment = decrypted.sublist(0, decrypted.length - 1);
  //     out.add(fragment);

  //     // Parse handshake messages inside this accumulated buffer
  //     final p = PeekableBuffer()..append(out.toBytes());

  //     while (true) {
  //       if (p.length < 4) break;

  //       // Peek handshake header
  ...
  //       // ✅ Hash EXACT handshake message
  //       final handshakeMessageBytes = Uint8List.fromList([
  //         ...headerBytes,
  //         ...payload,
  //       ]);
  //       _helloHash.add(handshakeMessageBytes);
  ...
  Future<Uint8List> _recvEncryptedHandshake(PeekableBuffer buf) async {
    ...
    while (true) {
      ...
      if (p.length >= 4) {
        final hh = HandshakeHeader.deserialize(p.read(4));

        // ✅ Replace constructor call with direct constant
        if (p.length >= hh.size && hh.messageType == 0x14) {
          break;
        }
      }
    }

...
class TLS13Session {
  ...
  Future<Uint8List> _recvEncryptedHandshake(PeekableBuffer buf) async {
    ...
    while (true) {
      // receive encrypted handshake record
      ...
      // strip TLSInnerPlaintext.content_type (0x16)
      final fragment = plain.sublist(0, plain.length - 1);
      out.add(fragment);

      // Python‑style: only detect Finished at START
      ...
      if (p.length >= 4) {
        ...
        if (hh.messageType == 0x14 && p.length >= 4 + hh.size) {
          // RETURN ENTIRE plaintext (Python style)
          return out.toBytes();
        }
      }
    }

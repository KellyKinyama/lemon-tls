...
dynamic r_u24(buf, off) {
  var v = ((buf[off] << 16) | (buf[off + 1] << 8) | buf[off + 2]) >>> 0;
  return (v, off + 3);
}

(List<int>, int) r_bytes(Uint8List buf, int off, int n) {
  Uint8List slice;
...
abstract class TlsExtension {
  ...
  TlsExtension(this.data, this.type);
...
class TlsExtensionSERVER_NAME extends TlsExtension {
  ...
  TlsExtensionSERVER_NAME(this.data, this.type) : super(data, type);
}
...
// /* --------------------------- SUPPORTED_VERSIONS (43) --------------------------- */
// exts.SUPPORTED_VERSIONS.encode = dynamic (value) {
...
// exts.SUPPORTED_VERSIONS.decode = dynamic (data) {
...
String ext_name_by_code(code) {
  ...
  for (TLS_EXT k in TLS_EXT.values) {
    if ((k.value >> 0) == (code >> 0)) return k.toString();
  }
  return 'EXT_' + code;
}
...
/* ================================ Hello I/O ================================ */
dynamic build_hello(kind, params) {
  params = params ?? {};
...
dynamic parse_hello(hsType, body) {
  var isClient =
      (hsType == TLS_MESSAGE_TYPE.CLIENT_HELLO || hsType == 'client_hello');
  ...
  if (isClient) {
    ...
    var extRaw = (body.length > off) ? body.subarray(off) : Uint8List(0);
    ...
    for (var k = 0; k < extensions.length; k++) {
      var e = extensions[k];
      if (e.type == TLS_EXT.SUPPORTED_VERSIONS && e.value is List) {
  ...
  var extRaw2 = (body.length > off) ? body.subarray(off) : Uint8List(0);
...
bool isVec2(u8) {
  if (!(u8 is Uint8List) || u8.length < 2) return false;
  var len = (u8[0] << 8) | u8[1];
...
dynamic build_certificate(params) {
  // Unified API:
  ...
  var entries = (params.entries is List) ? params.entries.slice() : null;
  if (!entries && params.certs is List) {
    // backward-compat: { certs:[...] } → entries:[{cert},...]
    entries = params.certs.map((c) {
      return (cert: c);
    });
  }
  if (!entries) entries = [];
  ...
  if (v == TLS_VERSION.TLS1_3) {
    var ctx = toU8(params.request_context ?? Uint8List(0));

    List<Uint8List> entryParts = [];
    for (var i = 0; i < entries.length; i++) {
      var certBytes = toU8(entries[i].cert ?? Uint8List(0));
      var certVec = veclen(3, certBytes); // cert_data vec<3>

      var extRaw = entries[i].extensions;
      if ((extRaw is List<TlsExtension>)) {
        ...
        extRaw = isVec2(extRaw) ? extRaw : veclen(2, extRaw);
      } else {
        // no extensions
        extRaw = veclen(2, Uint8List(0));
      }

      entryParts.addAll([certVec, extRaw]);
    }
    ...
    List<Uint8List> certListParts = [];
    for (var j = 0; j < entries.length; j++) {
      var c = toU8(entries[j].cert ?? Uint8List(0));
...
dynamic parse_certificate(Uint8List body) {
  // תמיד מחזיר:
  // {
  //   version: TLS_VERSION.TLS1_2 | TLS_VERSION.TLS1_3,
  ...
  if (body.length >= 4) {
    int off = 0;
    int rcLen;
    (rcLen, off) = r_u8(body, off); // 1 byte
    final afterCtx = off + rcLen; // off מצביע אחרי rcLen (כלומר 1)
    if (afterCtx + 3 <= body.length) {
      var listLen, off2 = afterCtx; // התחלת certificate_list
      (listLen, off2) = r_u24(body, off2); // 3 bytes
      if (afterCtx + 3 + listLen == body.length) {
        // נראה כמו 1.3 תקני
        final request_context = body.sublist(off, off + rcLen);
        off = off2;
        ...
        while (off < end) {
          var certLen;
          (certLen, off) = r_u24(body, off); // cert_data vec<3>
          var cert;
          (cert, off) = r_bytes(body, off, certLen);

          var extLen;
          (extLen, off) = r_u16(body, off); // extensions vec<2>
          var extRaw;
          (extRaw, off) = r_bytes(body, off, extLen);

          final extensions = extLen ? parse_extensions(extRaw) : [];
          entries.add({cert, extensions});
        }

        return (version: TLS_VERSION.TLS1_3, request_context, entries);
      }
    }
  }
  ...
  while (off3 < math.min(end2, body.length)) {
    ...
    entries12.add({cert}); // אין extensions ב-1.2
  }

  return (version: TLS_VERSION.TLS1_2, entries: entries12);
}
...
// } CertificateVerify;
Uint8List build_certificate_verify(scheme, signature) {
  var sig = toU8(signature ?? Uint8List(0));
  var alg = scheme >>> 0;

  var out = Uint8List(2 + 2 + sig.length);
  var off = 0;
...
dynamic parse_certificate_verify(body) {
  var off = 0;
...
// } NewSessionTicket;
Uint8List build_new_session_ticket(p) {
  var lifetime = ((p && p.ticket_lifetime) as int) >> 0;
  var age_add = ((p && p.ticket_age_add) as int) >> 0;
  var nonce = toU8(p && p.ticket_nonce ?? Uint8List(0));
  var ticket = toU8(p && p.ticket ?? Uint8List(0));
  var extsBuf = (p && p.extensions is List)
      ? build_extensions(p.extensions)
      : (p && p.extensions) ?? veclen(2, Uint8List(0));

  var out = Uint8List(
    (4 + 4 + 1 + nonce.length + 2 + ticket.length + extsBuf.length) as int,
  );
  var off = 0;
...
dynamic parse_new_session_ticket(body) {
  var off = 0;
...
dynamic build_certificate_request(params) {
  var v = params && params.version ?? TLS_VERSION.TLS1_3;

  if (v == TLS_VERSION.TLS1_3) {
    var ctx = toU8((params && params.request_context) ?? Uint8List(0));
    var extsBuf = params.extensions
        ? build_extensions(params.extensions)
        : (params && params.extensions) ?? veclen(2, Uint8List(0));
  ...
  // TLS 1.2 / 1.0 / 1.1
  var typesArr =
      ((params && params.certificate_types) ?? [1])
          as List; // rsa_sign(1) as default
  var typesBuf = Uint8List(typesArr.length);
  for (var i = 0; i < typesArr.length; i++) typesBuf[i] = typesArr[i] & 0xFF;
  var typesVec = veclen(1, typesBuf);

  var sigalgs = ((params && params.signature_algorithms) ?? []) as List;
  var sigBuf = Uint8List(sigalgs.length * 2);
  var o = 0;
  for (var j = 0; j < sigalgs.length; j++) o = w_u16(sigBuf, o, sigalgs[j]);
  var sigVec = sigalgs.isNotEmpty ? veclen(2, sigBuf) : Uint8List(0);

  var cas = ((params && params.certificate_authorities) ?? []) as List;
  List<Uint8List> caParts = [];
  var caTotal = 0;
  for (var k = 0; k < cas.length; k++) {
    var dn = toU8(cas[k]);
    var ent = Uint8List(2 + dn.length);
    var oo = 0;
...
dynamic parse_certificate_request(body) {
  ...
  for (var i = 0; i < typesBytes.length; i++)
    certificate_types.add(typesBytes[i] >>> 0);
...
Uint8List build_hello_retry_request(params) {
  // params: { cipher_suite, selected_version, selected_group, cookie?: Uint8Array|string, other_exts?: list }
  // throw Exception("TLS13_HRR_RANDOM is not defined");
  var rnd = Uint8List(0); //TLS13_HRR_RANDOM;
  var sid = Uint8List(0);
  var legacy_version = TLS_VERSION.TLS1_2;

  List<TlsExtension> extList = [];
  // supported_versions (selected)
  // throw UnimplementedError();
  // extList.add(TlsExtension(
  //     value: ++(params && params.selected_version) ?? TLS_VERSION.TLS1_3
  ...
  // cookie if supplied
  if (params && params == TLS_EXT.COOKIE) {
    // if (params == TLS_EXT.COOKIE) {
    throw UnimplementedError(params.cookie);
    // exts.COOKIE = {
    //   encode: (v) {
    //     return veclen(2, toU8(v ?? ''));
    //   },
    //   decode: (d) {
    //     var off = 0, v;
    //     [v, off] = readVec(d, 0, 2);
    ...
    // };
    // }
    // extList.add((type: 'COOKIE', value: params.cookie));
  }
  // other extensions passthrough
  if (params && (params.other_exts is List)) {
    for (var i = 0; i < params.other_exts.length; i++)
      extList.add(params.other_exts[i]);
  }

  var extsBuf = build_extensions(extList);
  var cipher_suite = (params && params.cipher_suite is num)
      ? params.cipher_suite
      : 0x1301;

  // Wire = legacy_version + random + sid + cipher_suite + compression(0) + extensions
  var out = Uint8List(2 + 32 + 1 + 0 + 2 + 1 + extsBuf.length);
  var off = 0;
  off = w_u16(out, off, legacy_version);
...
//   }
dynamic build_server_key_exchange_ecdhe(p) {
  // p: { group:u16, public:Uint8Array|string, sig_alg:u16, signature:Uint8Array|string }
  var pub = toU8(p.public ?? 0);

  var head = Uint8List(1 + 2 + 1 + pub.length);
  var off = 0;
  ...
  var sig = toU8(p.signature ?? 0);
  var sigpart = Uint8List(2 + 2 + sig.length);
  var o2 = 0;
...
dynamic parse_server_key_exchange(body) {
  var off = 0;
  var curve_type;
...
dynamic build_client_key_exchange_ecdhe(pubkey) {
  var p = toU8(pubkey ?? 0);
  return veclen(1, p);
}

dynamic parse_client_key_exchange_ecdhe(body) {
  var off = 0;
  var v;
  (v, off) = readVec(body, 0, 1);
  return v;
}

dynamic build_client_key_exchange_rsa(enc_pms) {
  var e = toU8(enc_pms ?? 0);
  return veclen(2, e);
}

dynamic parse_client_key_exchange_rsa(body) {
  var off = 0;
  var v;
  [v, off] = readVec(body, 0, 2);
  return v;
}
...
// } NewSessionTicket;
dynamic build_new_session_ticket_tls12(p) {
  var hint = ((p && p.ticket_lifetime_hint) as int) >> 0;
  var ticket = toU8(p && p.ticket ?? Uint8List(0));
  var out = Uint8List(4 + 2 + ticket.length);
  var off = 0;
...
dynamic parse_new_session_ticket_tls12(body) {
  var off = 0;
  var hint =
      (body[off] << 24 |
          body[off + 1] << 16 |
...
/* ============================= Handshake message ============================= */
dynamic build_message(params) {
  var type = 0;
  var body = null;

  if (params.type == 'server_hello') {
    type = TLS_MESSAGE_TYPE.SERVER_HELLO.value;
...
dynamic build_extensions(exts){
  ...
  return Uint8List(out);
}

dynamic parse_extensions(data) {
  var ptr = 0;
...
dynamic parse_client_hello(data) {
  var ptr = 0;
  ...
  for (var ext of extensions) {
    var ext_data = Uint8List(ext.data);
...
dynamic build_server_hello(params){
  ...
  // 3) legacy_session_id
  var sid = params.legacy_session_id || Uint8List(0);
